...
public class DialogAfegirSimptoma extends javax.swing.JDialog {

    private ControladorPresentacioSimptomes controladorSimptomes;
    private DefaultListModel model;
    private List<String> simtomesSeleccionats;
    private boolean acceptat;

    public DialogAfegirSimptoma(ControladorPresentacioSimptomes controladorSimptomes) {
        super();

        this.controladorSimptomes = controladorSimptomes;
        simtomesSeleccionats = new ArrayList<String>();
        ...
        jListSimptomes.setCellRenderer(new CheckListRenderer());
        jListSimptomes.setModel(model);
        omplirModel();

        jListSimptomes.addMouseListener(new MouseAdapter() {
            ...
            public void mouseClicked(MouseEvent event) {
                ...
                if (index != -1) {
                    ...
                    if (!item.isSelected) {
                        if (!simtomesSeleccionats.contains(labelItem)) {
                            simtomesSeleccionats.add(labelItem);
                        }
                    } else {
                        simtomesSeleccionats.remove(labelItem);
                    }

                    item.setSelected(!item.isSelected);

                    jListSimptomes.repaint(jListSimptomes.getCellBounds(index, index));
                }
            }
        });
    ...
    private void initComponents() {
        ...
        setTitle("Afegir Simptomes");

        jLabel1.setFont(new java.awt.Font("Dialog", 1, 15)); // NOI18N
        jLabel1.setText("Símptomes:");

        jListSimptomes.setFont(new java.awt.Font("Dialog", 0, 15)); // NOI18N
        jListSimptomes.setModel(new javax.swing.AbstractListModel() {
    ...
    public List<String> getSimtomesSeleccionats() {
        return simtomesSeleccionats;
    }

    private void omplirModel() {
        for (String nomSimptoma : controladorSimptomes.getNomSimptomes()) {
            model.addElement(new CheckListItem(nomSimptoma));
        }
    }
