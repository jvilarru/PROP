...
public class DialogAfegirPacient extends javax.swing.JDialog {

    private ControladorPresentacioPacients controladorPacients;
    private DefaultListModel model;
    private List<String> pacientsSeleccionats;
    private boolean acceptat;

    public DialogAfegirPacient(ControladorPresentacioPacients controladorPacients) {
        super();

        this.controladorPacients = controladorPacients;
        pacientsSeleccionats = new ArrayList<String>();
        acceptat = false;
        ...
        jListPacients.setCellRenderer(new CheckListRenderer());
        jListPacients.setModel(model);
        omplirModel();

        jListPacients.addMouseListener(new MouseAdapter() {
            ...
            public void mouseClicked(MouseEvent event) {
                ...
                if (index != -1) {
                    ...
                    if (!item.isSelected) {
                        if (!pacientsSeleccionats.contains(labelItem)) {
                            pacientsSeleccionats.add(labelItem);
                        }
                    } else {
                        pacientsSeleccionats.remove(labelItem);
                    }

                    item.setSelected(!item.isSelected);

                    jListPacients.repaint(jListPacients.getCellBounds(index, index));
                }
            }
        });
    ...
    private void initComponents() {
        ...
        setTitle("Afegir Pacients");

        jLabel1.setFont(new java.awt.Font("Dialog", 1, 15)); // NOI18N
        jLabel1.setText("Pacients");

        jListPacients.setFont(new java.awt.Font("Dialog", 0, 15)); // NOI18N
        jListPacients.setModel(new javax.swing.AbstractListModel() {
    ...
    public List<String> getPacientsSeleccionats() {
        return pacientsSeleccionats;
    }

    private void omplirModel() {
        for (String pacient : controladorPacients.getDniNomCognomPacients()) {
            model.addElement(new CheckListItem(pacient));
        }
    }
